...
export class WenzhangApi {
    ...
    public wenzhangdetail(data, showLoadingModal: boolean = true) {
        var url = ApiConfig.getApiUrl() + 'wenzhang/wenzhangdetail';
        var headers = ApiConfig.GetHeader(url, data);
        let options = new RequestOptions({ headers: headers });
        let body = ApiConfig.ParamUrlencoded(data);
        let loading = null;
        ...
        return this.http.post(url, body, options).toPromise()
            .then((res) => {
                if (ApiConfig.DataLoadedHandle('wenzhang/wenzhangdetail', data, res)) {
                    if (showLoadingModal) {
                        ApiConfig.DimissLoadingModal();
                    }
                    if (res==null) {
            ...
            .catch(err => {
                ...
                return ApiConfig.ErrorHandle('wenzhang/wenzhangdetail', data, err);
            });
    }

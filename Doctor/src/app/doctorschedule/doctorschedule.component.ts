...
export class DoctorscheduleComponent extends AppBase  {
  ...
  setDate(timetable,date,time){
    console.log(this.weekschedule,'week');
    console.log(date,'week');
    console.log(time,'week');
    var dt=date.dt+time.hour+time.minute;


    if(timetable[dt]==undefined){
     timetable[dt]={rd:dt,fdate:date.datestr,ftime:time.hour+":"+time.minute,doctor_id:this.params.doctor_id,bookingcount:0};
    }
    if(timetable[dt].bookingcount>0){
      return;
    }
    if(time.pass==true){
      return;
      ...
      timetable[dt].isleisure=timetable[dt].isleisure=='Y'?"N":"Y";
    
      console.log(timetable[dt],'time')
      this.operatorApi.settime(timetable[dt]).then((ret:any)=>{
        console.log(ret);
      });
  ...
  overtime=0;
  overday="";
  setMouseOver(date,time){
    this.overdoctor=this.params.doctor_id;
